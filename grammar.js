  ...
  rules: {
    ...
    class_decl: ($) =>
      seq(
        ...
        optional(
          seq("(", optional(field("parameters", $.var_param_decls)), ")"),
        ),
        optional(
          seq(
            "extends",
    ...
    var_member: ($) =>
      seq(optional("private"), "var", field("decls", $.var_decls), ";"),
    def_member: ($) =>
      seq(
        optional("private"),
        "def",
    ...
    new_member: ($) =>
      seq(
        "new",
        "(",
        optional($.new_param_decls),
        ")",
        optional(seq(optional(":"), "super", $.tuple_expr)),
        $.block_stmt,
      ),
    def_method: ($) =>
      seq(optional("private"), "def", choice($.index_method, $.method)),
    variant_case: ($) =>
      seq(
        "case",
        field("name", $.identifier),
    ...
    enum_cases: ($) => seq($.enum_case, repeat(seq(",", $.enum_case))),
    ...
    var_param_decls: ($) =>
      seq($.var_param_decl, repeat(seq(",", $.var_param_decl))),
    param_decl: ($) =>
      seq(field("name", $.identifier), ":", field("type", $.type_ref)),
    param_decls: ($) => seq($.param_decl, repeat(seq(",", $.param_decl))),
    new_param_decl: ($) =>
      seq(
        field("name", $.identifier),
        ...
        seq(
          choice(
            ...
            seq(
              field(
                "member",
                seq($.ident_param, repeat(seq(".", $.ident_param))),
              ),
            ),
          ),
    ...
    var_decls: ($) => seq($.var_decl, repeat(seq(",", $.var_decl))),
    index_method: ($) =>
      seq(
        $.ident_param,
        "[",
    ...
    method: ($) =>
      seq(
        field("name", $.ident_param),
        "(",
        optional(field("parameters", $.var_param_decls)),
        ")",
    ...
    param_expr: ($) => "_",
    if_expr: ($) =>
      seq("if", "(", $.expr, ",", $.expr, optional(seq(",", $.expr)), ")"),
    literal: ($) => choice($.const, "this", $.ident_param),
    const: ($) =>
      choice($.char, $.string, $.integer, $.float, "true", "false", "null"),
